...
7. **Duty log (voice sitrep).** The officer talks the day in (`/sitrep`, `sitrep.*`). The transcript is a work artefact they chose to file. Follow-up questions complete the log; a **private** tone/mood heuristic (labels only, not a clinical score) stays on `/me/sitreps` and never enters the heatmap. Another commander cannot read these rows. Seeded dummy logs exist for `commander.3bn`.
